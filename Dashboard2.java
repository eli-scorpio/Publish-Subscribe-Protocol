...
public class Dashboard2{
    ...
    public static void main(String[] args) throws Exception {
        dashboardSocket = new DatagramSocket(Constants.DASHBOARD2_PORT);
    ...
    private static void userPublish() throws IOException {
        ...
        System.out.println("[DASHBOARD2] CHOOSE TOPIC TO PUBLISH");
        System.out.println("Select Topic:\n2 BLINDS\n3 AC");
        while(!sc.hasNextInt()) {
            System.out.println("[DASHBOARD2] USER ERROR: Select query 2 or 3 by typing the respective number");
            System.out.println("Select Topic:\n2 BLINDS\n3 AC");
        ...
        if(topic == Constants.BLINDS || topic == Constants.AC) {
            System.out.print("[DASHBOARD2] Type command and press enter to publish > ");
            String command = sc.nextLine();
            publish(topic, command);
        }
        else
            System.out.println("[DASHBOARD2] ERROR: Invalid topic number!");
    }

    // publish to a topic
    private static void publish(int topic, String message) throws IOException {
        System.out.println("[DASHBOARD2] Publishing to " + ((topic == Constants.BLINDS)? "BLINDS":
        ...
        System.out.println("[DASHBOARD2] Waiting for ACK...");
        while(packet.getType() != Constants.ACK) {
            System.out.println("[DASHBOARD2] ERROR - ACK not received! Re-Publishing to BROKER" );
            sendPacket(Constants.DATA, Constants.PUBLISH, topic, message);
    ...
    private static int askTopics(){
        Scanner sc = new Scanner(System.in);
        System.out.println("[DASHBOARD2] SELECT A TOPIC TO SUBSCRIBE TO");
        System.out.println("Select Topic:\n0 Temperature\n1 DateTime");
        while(!sc.hasNextInt()) {
            System.out.println("[DASHBOARD2] USER ERROR: Select query 0,1 or 2 by typing the respective number");
            System.out.println("Select Topic:\n0 Temperature\n1 DateTime");
            sc.nextLine(); // flush scanner
        }
        return sc.nextInt();
    ...
    private static int askQuery(){
        Scanner sc = new Scanner(System.in);
        System.out.println("[DASHBOARD2] SELECT A QUERY");
        System.out.println("Select Topic:\n0 PUBLISH\n1 SUBSCRIBE");
        while(!sc.hasNextInt()) {
            System.out.println("[DASHBOARD2] USER ERROR: Select query 0,1 or 2 by typing the respective number");
            System.out.println("Select Topic:\n0 PUBLISH\n1 SUBSCRIBE");
